...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    if (user_id === provider_id) {
      return res
        .status(400)
        .json({ error: 'You cannot make an appointment with you!!!' });
    }

    // check if provider is a provider
    // falta verificar se o provider num esta marcando com ele mesmo
    ...
    /**
     * Ckeck if appointment date is not past date
     */
  ...
  async delete(req, res) {
    ...
    if (isBefore(dateWithSub, new Date())) {
      return res.status(401).json({
        error: 'You can only canlcel appointments 2 hours in advance!!!',
      });
    }
